...
# Number of generations to run for, no limit if =0
k_n_generations = 0
...
class App(object):
    ...
    """
    def __init__(self,time_limit, n_generations, pop_size):
        '''
        ...
        self.GA_loop(time_limit, n_generations, pop_size)

    def GA_loop(self, time_limit, n_generations, pop_size):
        '''
        ...
        print("Elapsed time was {0:.1f} seconds.".format(elapsed_time))
        print(' ')
...
def specific_cities2():
    ...
    try:
        start_time = time.time()
        app = App(time_limit=TIME_LIMIT, n_generations=k_n_generations, pop_size=k_population_size)
